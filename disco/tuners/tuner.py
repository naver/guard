...
class Tuner():
    ...
    default_params = {
        ...
        "scheduler": "constant",
        "warmup_steps": 2**6, # MB changed from 2**6 or 1
    ...
    def __init__(self, model, target, proposal=None, context_distribution=SingleContextDistribution(), loss=JSLoss(), features=[], 
            track_metrics=["kl", "tv", "js"], track_divergence_from_base=False, system_prompt="", **params):
        """
        Parameters
        ----------
        model: distribution
    ...
    def _update_moving_z(self, proposal_log_scores, target_log_scores, context):
        ...
        self.z[context] += z_pointwise_estimates.to('cuda')
...
#             print("Locks proposal updating") ## MB
            self.proposal.network.load_state_dict(self.model.network.state_dict()) ## MB
    ...
    def _step(self):
        ...
        for context in contexts:
            ...
            samples, proposal_log_scores = sampler.sample(sampling_size=self.params["sampling_size"], context=self.system_prompt + context)
...
#                 if  0 == (s + 1) % 20:
#                     self.model.save(path="models/joy/dpg-"+str(int((s+1)*5))+"k")
#                     self.model.save(path="models/happy/dpg-"+str(int((s+1)*2))+"k3")
