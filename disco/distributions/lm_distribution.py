...
import torch
import copy
from transformers import AutoTokenizer, AutoModelForCausalLM, AutoModelForSeq2SeqLM, LogitsProcessorList, T5ForConditionalGeneration
...
class LMDistribution(BaseDistribution):
    ...
    def __init__(self,
            ...
            length=30, device="cuda:0", token="", LLM=False,
            **config
        ):
        """
        Parameters
        ...
        token: huggingface token_id
            Minbeom added it for using restricted models
        config: kwarg
        ...
        if token=="":
            ...
            self.tokenizer= AutoTokenizer.from_pretrained(tokenizer if tokenizer else model, token=token)
#         assert auto in [AutoModelForCausalLM, AutoModelForSeq2SeqLM, T5ForConditionalGeneration], "only AutoModel, AutoModelForCausalLM and AutoModelForSeq2SeqLM are valid options."
#         if 'gpt2' in model:
#             self.tokenizer.pad_token_id = self.tokenizer.eos_token_id
#             print('pad_token_id is set to eos_token_id')
        if LLM:
            self._load_network_llm(auto, model, token)
        else:
            self._load_network(auto, model, token)
        ...
        if self.tokenizer.bos_token is None: # mb
    ...
    def save(self, path=""):
        self.network.save_pretrained(path)
        self.tokenizer.save_pretrained(path)

    def _load_network(self, auto, model, token):
        if token == "":
            self.network = auto.from_pretrained(model)
        else:
            self.network = auto.from_pretrained(model, token=token)
            
        if not self.network.config.is_encoder_decoder:
            self.tokenizer.pad_token = self.tokenizer.eos_token
            self.network.config.pad_token_id = self.tokenizer.eos_token_id

    def _load_network_llm(self, auto, model, token):
        self.network = auto.from_pretrained(model, device_map="auto", torch_dtype=torch.bfloat16, token=token)
#         if not self.network.config.is_encoder_decoder:
#             self.tokenizer.pad_token = self.tokenizer.eos_token
#             self.network.config.pad_token_id = self.tokenizer.eos_token_id
    ...
    def sample(self, context="", sampling_size=32, sum=True, qa_concat=False):
...
#             max_new_tokens=350, # MB makes it for analyzing open-ended text generation
# #             max_new_tokens=self.length, # origin code
#             do_sample=True, pad_token_id=self.tokenizer.eos_token_id, **generate_kwargs)
        
        # MB length constraint
        outputs = self.network.generate(input_ids,
            output_scores=True, return_dict_in_generate=True,
            max_new_tokens=self.length, min_length=self.length,
            do_sample=True, pad_token_id=self.tokenizer.eos_token_id, **generate_kwargs)
