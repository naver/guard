...
class BasicRejectionSampler(Sampler):
    """
    Basic-Sampling class by minbeom
    """

    def __init__(self, proposal, constraints):
        """
        Parameters
        ----------
    ...
    def sample(self, sampling_size=32, context=''):
        ...
        results = self.constraints.score(samples, context).to(device)

        accepted_samples = [x for k, x in zip(results, samples) if k==1.0]
        self.n_accepted_samples += len(accepted_samples)
        print('AR =', len(accepted_samples)/self.n_samples)
        # print(self.n_accepted_samples, self.n_samples)
